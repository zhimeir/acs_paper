...
params = {'setting': [1, 2, 3, 4, 5], 'sig': np.linspace(1, 5, num = 5), 'seed_grp': range(50), 'method': ['SVR', 'GB', 'RF'], 'size': [200, 500, 1000]}
...
for i in range(seed_num):
    ...
    acs_res = acs_obj.filter(Xtrain, Ytrain, Xcalib, Ycalib, Xtest, batch_size = batch_size, prescreen=False)
...
all_res.to_csv(git_root+"/results/base_setting"+str(set_id)+"n"+str(n)+"sig"+str(sig)+method+"seed"+str(seed_grp)+".csv")
