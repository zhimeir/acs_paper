...
params = {'model': ['trained'], 'data': ['cxr'], 'N': [100, 500, 1000], 'split': [1, 3], 'split_tune': [2]}
params_grid = list(ParameterGrid(params))


parser = argparse.ArgumentParser()
parser.add_argument('--repN', type=int, default=100)
...
split_pr = split / 10
num_gens = 10
acc_name = 'generations|chexbert|acc'
...
if split_pr_tune < 1 - split_pr:
    ...
    if not os.path.isdir(f"./acs_en_results/save_scores"):
        os.makedirs(f"./acs_en_results/save_scores")
    if not os.path.exists(f'./acs_en_results/save_scores/scores_{model}_{data}_{str(N)}_{str(split)}_{str(split_tune)}.npy'):
        np.save(f'./acs_en_results/save_scores/scores_{model}_{data}_{str(N)}_{str(split)}_{str(split_tune)}.npy', uq_res)
        np.save(f'./acs_en_results/save_scores/labels_{model}_{data}_{str(N)}_{str(split)}_{str(split_tune)}.npy', np.array(label))
    ...
    for q in q_seq:
        ...
        for seed in tqdm.tqdm(range(repN)):
            ...
            """ ADACS """
            # adacs_obj = adacs(q, 0, mdl_list, self_train = False)
            adacs_obj = acs(q, 0, mdl_list, self_train = False)
            adacs_res = adacs_obj.filter(Xtrain, Ytrain, Xcalib, Ycalib, Xtest, batch_size = 10)
            adacs_res = adacs_res.astype(bool)
            if sum(adacs_res) == 0:
                adacs_res_fdp = 0
                adacs_res_power = 0
            else:
                adacs_res_fdp = np.sum(Ytest[adacs_res] <= 0) / sum(adacs_res)
            ...
            for method in mdl_list:
                ...
                if mse_new < mse:
                    winner = method
                    mse = mse_new
                    winner_fdp = BH_cs_fdp
                    winner_power = BH_cs_power
        ...
        out_filename = f"./acs_en_results/model{model}_data{data}/model{model}_data{data}_N{str(N)}_q{str(int(q*100))}_split{int(split)}_splittune{split_tune}"
        if not os.path.isdir(f"./acs_en_results/model{model}_data{data}"):
            os.makedirs(f"./acs_en_results/model{model}_data{data}")
