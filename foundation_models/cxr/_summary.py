...
split_pr = 0.5
num_thresholds = 50
overall_res, individual_res = o.get_uq_all(name=uq, uq_list=uq_list, acc_name=acc_name, num_gens=num_gens, uq_kwargs_ref=uq_kwargs_ref, uq_kwargs=uq_kwargs)
...
individual_res.to_csv(f'{par_path}/uq_result/scores_{tau}.csv', index=True)  
pd.DataFrame(labels).to_csv(f'{par_path}/uq_result/labels_{tau}.csv', index=False) 
