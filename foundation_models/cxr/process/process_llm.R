      ...
      for(split_tune_id in c(2)){
        if(split_id + split_tune_id < 10){
          ...
          for(size in c(100,500,1000)){
            for(seed_grp in 1:num_batch - 1){
              for(regressor in regressor_list){
                for(q_id in 5*(1:9)){
                  filename = sprintf("./acs_results/model%s_data%s/model%s_data%s_N%d_q%d_split%d_splittune%d_method%s.csv", model, data, model, data, size, q_id, split_id, split_tune_id, regressor)
          ...
          sum_res = all_res %>% group_by(level, method, regressor, model, size) %>%
            mutate(method = factor(method, c("adacs", "CS"), c("ACS  ", "CS"))) %>%
          ...
          main_dir = sprintf("./figs/acs/model%s_data%s", model, data)
          plot_dir = sprintf("./figs/acs/model%s_data%s_split%d_splittune%d.pdf", model, data, split_id, split_tune_id)
