...
class UQ_computer:
    ...
    def get_semantic_entropy(self, num_gens:int, normalize:bool):
        ...
        print("load likelihoods")
        # for idx in tqdm.tqdm(range(2,self.batch_num+1)):
        #     cleaned_sequences = read_cleaned_outputs_new(self.path)[:5 if debug else None]
        #     name = f'read_semantic_similarities#{key}'
        #     os.makedirs(os.path.join(os.path.dirname(self.path), 'logs'), exist_ok=True)
        #     logger = utils.get_logger(name, os.path.join(os.path.dirname(self.path), f'logs/{name}.log'), propagate=False)
        #     model, tokenizer = models.load_model_and_tokenizer(_get_model_name(self.path), device)
        #     likelihoods = lw._get_loglikelihoods(cleaned_sequences[((idx-1) * self.batch_size) : (idx * self.batch_size)], model, tokenizer, clean=clean, logger=logger)
