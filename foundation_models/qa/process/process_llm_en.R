        ...
        for(data in c("triviaqa", "coqa")){
          ...
          for(size in c(100,500,1000)){
            for(seed_grp in 1:num_batch - 1){
              for(q_id in 5*(1:9)){
                filename = sprintf("./acs_en_results/model%s_data%s/model%s_data%s_N%d_q%d_split%d_splittune%d.csv", model, data, model, data, size, q_id, split_id, split_tune_id)
                new_res = read_csv(filename)[,-1] %>% mutate(level = q_id / 100, size = size)
          ...
          head(all_res)
          sum_res = all_res %>% group_by(level, method, size) %>%
            mutate(method = factor(method, c("adacs", "rf", "logistic", "xgbrf", "naive"), c("ACS ", "CS-RF ", "CS-logistic", "CS-XGBRF  ", "naive "))) %>%
          ...
          pp1 = sum_res %>%
            ...
            geom_bar(stat = "identity", position = position_dodge(0.035), alpha = 0.7, width = 0.02) +
            theme_bw() +
            # geom_point() +
            # geom_line() +
            # theme_bw() +
          ...
          print(both)
          ggsave(sprintf("./figs/acs_en/ensemble_model%s_data%s_split%d_splittune%d.pdf", model, data, split_id, split_tune_id),
