...
# params = {'model': ['llama-2-13b-chat-hf'], 'data': ['triviaqa'], 'N': [100, 500, 1000], 'split': [1,3], 'split_tune': [2]}
...
for method in mdl_list:
    ...
    if split_pr_tune < 1 - split_pr:
        ...
        if not os.path.isdir(f"./acs_results/save_scores"):
            os.makedirs(f"./acs_results/save_scores")
        if not os.path.exists(f'./acs_results/save_scores/scores_{model}_{data}_{method}_{str(N)}_{str(split)}_{str(split_tune)}.npy'):
            np.save(f'./acs_results/save_scores/scores_{model}_{data}_{method}_{str(N)}_{str(split)}_{str(split_tune)}.npy', uq_res)
            np.save(f'./acs_results/save_scores/labels_{model}_{data}_{method}_{str(N)}_{str(split)}_{str(split_tune)}.npy', np.array(label))
        ...
        for q in q_seq:
            ...
            out_filename = f"./acs_results/model{model}_data{data}/model{model}_data{data}_N{str(N)}_q{str(int(q*100))}_split{int(split)}_splittune{split_tune}_method{method}"
            if not os.path.isdir(f"./acs_results/model{model}_data{data}"):
                os.makedirs(f"./acs_results/model{model}_data{data}")
